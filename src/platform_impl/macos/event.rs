...
use crate::{
    ...
    platform_impl::platform::{ffi, util::Never, DEVICE_ID},
};
...
fn get_logical_key_char(ns_event: &NSEvent, modifierless_chars: &str) -> Key<'static> {
    let string = ns_event.charactersIgnoringModifiers().map(|s| s.to_string()).unwrap_or_else(String::new);
    if string.is_empty() {
        // Probably a dead key
...
pub(crate) fn create_key_event(
    ...
    let text_with_all_modifiers: Option<&'static str> = {
        if key_override.is_some() {
            ...
            let characters = ns_event.characters().map(|s| s.to_string()).unwrap_or_else(String::new);
    ...
    if !matches!(key_from_code, Key::Unidentified(_)) {
        ...
        key_without_modifiers = key_from_code.clone();
    } else {
...
pub(super) fn modifier_event(
    ...
    if !was_key_pressed && ns_event.modifierFlags().contains(keymask)
        || was_key_pressed && !ns_event.modifierFlags().contains(keymask)
    {
        let mut event = event.clone();
        event.state = if was_key_pressed {
